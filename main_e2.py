...
from main_ds2 import send_start_email
...
def send_email_sendgrid(subject, body):
    ...
    if not api_key or not to_email or not from_email:
        logger.warning("SendGrid env not configured")
        return
    ...
    logger.info(f"sendgrid response: {r.status_code} {r.text}")
...
def start_handler(message):
    ...
    if not is_member(user_id):
        ...
        # حتی اگر عضو نبودیم هم ممکنه بخوایم ایمیل بزنیم؛ در اینجا فقط وقتی عضو نیست ایمیل نزنیم.
        return
